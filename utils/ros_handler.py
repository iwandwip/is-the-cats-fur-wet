...
class RosHandler:
    ...
    ballFilter = rospy.Publisher(
        "ball/coordinate_filter", Float32MultiArray, queue_size=10)
    ballCoordinate = rospy.Publisher(
        "ball/coordinate", Int16MultiArray, queue_size=10)
    ballState = rospy.Publisher(
        "ball/ball_state", Int8MultiArray, queue_size=10)
    frameSize = rospy.Publisher(
        "frame/frame_size", Int32MultiArray, queue_size=10)
    ...
    def ball2ros(self, data, shape):
        try:
            ...
            if flag:
                ...
                position.layout.dim[0].stride = 1
                coordinate.data = [x_filter, y_filter, distance]
                position.data = [int(data["x_pos"]), int(
                    data["y_pos"]), int(data["size"] // 2)]
