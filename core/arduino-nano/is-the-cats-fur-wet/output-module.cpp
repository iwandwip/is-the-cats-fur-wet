...
void DigitalOut::on() {
        if (_reverse) digitalWrite(_pin, LOW);
        else digitalWrite(_pin, HIGH);
        _state = true;
}

void DigitalOut::off() {
        if (_reverse) digitalWrite(_pin, HIGH);
...
void DigitalOut::update() {
        if (_onDelayState) {
                ...
                if (currentMillis - _onDelayStart >= _onDelayTime) {
                        on();
                        _onDelayState = false;
                }
        }
        if (_offDelayState) {
                unsigned long currentMillis = millis();
                if (currentMillis - _offDelayStart >= _offDelayTime) {
                        off();
                        _offDelayState = false;
                }
        }
